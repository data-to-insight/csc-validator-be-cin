...
def validate(
    ...
    df_cin_ref = data_container[Cindetails].copy()
    
    # Before you begin, rename the index so that the initial row positions can be kept intact.
    df_cin_ref.index.name = "ROW_ID"
    ...
    df_cin_ref.reset_index(inplace=True)
    ...
    # Issues dfs should return rows where CINreferralDate is greater than the Reference Date.

    #  Create dataframes which only have rows with CP plans, and which should have one plan per row.
    df_cin_ref = df_cin_ref[df_cin_ref[CINreferralDate].notna()]
