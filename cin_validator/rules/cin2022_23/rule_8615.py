...
def validate(
    ...
    df["S47ActualStartDate"] = pd.to_datetime(
        df["S47ActualStartDate"], format=r"%d-%m-%Y", errors="coerce"
    )
    df["DateOfInitialCPC"] = pd.to_datetime(
        df["DateOfInitialCPC"], format=r"%d-%m-%Y", errors="coerce"
    )
