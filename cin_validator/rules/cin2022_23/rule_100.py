...
def validate(
    ...
    error_date = df[ReferenceDate] != pd.to_datetime('31/03/2022',format='%d/%m/%Y',errors='coerce')
    ...
    rule_context.push_issue(
        table=Header, field=ReferenceDate, row=failing_indices
    )


def test_validate():
    # Create some sample data such that some values pass the validation and some fail.
    header = pd.DataFrame([['31/03/2022'], [pd.NA], [pd.NA],['30/11/2021']], columns=[ReferenceDate])
