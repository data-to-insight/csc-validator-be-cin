...
def validate(
    ...
    df[InitialCPCtarget] = pd.to_datetime(df[InitialCPCtarget], format='%d-%m-%Y', errors = 'coerce')
    print(df)
...
def test_validate():
    ...
    child_identifiers = pd.DataFrame([['17-06-2021'], ['09-10-2022'], ['14-03-2020']], columns=[InitialCPCtarget]) # 9/10 is a Sunday and 14/3 is a Saturday
    ...
    assert result.definition.message == "Please check: The Target Date for Initial Child Protection Conference should not be a weekend"