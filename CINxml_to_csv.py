    ...
    for group in child: 
        ...
        for element in group: 
            ...
            if element_count in element_counts:
                ...
                if element in ["Assessments", "ChildProtectionPlans"]:
                    subs_counts = {"ReviewsCount":0, "FactorsIdentifiedAtAssessmentCount":0}
                    for sub in element:
                        ...
                        sub.set(sub_count, subs_counts[sub_count])
                        print(f"{sub.tag}:{sub.attrib}")
                        subs_counts[sub_count] += 1  
                element_counts[element_count] += 1    
        group_counts[group_count] += 1
    # only change the count at the end to ensure that all childCount values within a block are the same.
    child_count += 1
